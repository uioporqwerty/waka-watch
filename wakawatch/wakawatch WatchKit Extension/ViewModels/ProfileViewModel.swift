...
final class ProfileViewModel: ObservableObject {
    ...
    func getProfile(user: UserData?) async throws {
        if user == nil {
            ...
            let leaderboardData = try await networkService.getLeaderboard(boardId: nil, page: nil)
            ...
            self.analytics.setProfile(properties: [
                ...
                "$name": !(profile.data.full_name?.isEmpty ?? true) ?
                            profile.data.full_name : profile.data.display_name,
